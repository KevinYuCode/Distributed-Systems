...
int main(int argc, char *argv[])
{
    ...
    const string fileName = "../leaving_AD.csv";
    ...
    // ky: aircraft is the data structure to hold information about each new aircraft object that is read from the csv
    while (file >> aircraft)
    {
        if (lastUsedTime == 0)
        {
        ...
        // ky: Remember tha all aircraft is just a map that keeps track of instance handles (aka the primary key for that aircraft)
        ...
        {
            // not there
            // KY: h is an instance of aircraft data
