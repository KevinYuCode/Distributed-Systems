...
int main(int argc, char *argv[])
{
    ...
    // Adding a SET to keep track of which planes we've made notifications for
    ...
    ctopic->filter_function([](const ControlTransfer::Update &sample)
                            { return sample.to() == ControlTransfer::FlightZone::AD; }); // fix this to only get to: ad
    ...
    while (1)
    {
        ...
        try
        {
            // cout << "Waiting for the homies" << endl;

            // wait for more data or for the publisher to end.
            waitset.wait(dds::core::Duration::infinite());
            // controlWaitset.wait(dds::core::Duration::infinite()); //TODO FIX THE WAITSET

            // cout << "Hello guys welcome back" << endl;
        }
                ...
                if (info.valid())
                {
                    ...
                    if (msg.baroaltitude() > 0.0)
                    {
                        ...
                        if (distance <= 8 && meterToFeet(msg.baroaltitude()) < 3000 && msg.vertrate() >= 0)
                        {
                            ...
                            ControlTransfer::Update ADUpdate(
                                msg.callsign(),                      // string callsign;
                                ControlTransfer::FlightZone::CENTRE, // FlightZone zone
                                ControlTransfer::FlightZone::AD,
                                msg.timestamp() // unsigned long timestamp;
                            );

                            controlTransferWriter.write(ADUpdate);
                        }
                    }
                }
            }
